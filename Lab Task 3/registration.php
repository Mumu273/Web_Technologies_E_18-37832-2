...
$errCount = 0;
    ...
    $errCount = $errCount + 1;
     ...
     if (empty($_POST["password"])) {
         ...
         $errCount = $errCount + 1;
          ...
          if (empty($cnfrmPass)) {
               ...
               if ($password != $cnfrmPass) {
                    ...
                    $confrmPassErr = "Confirm password didn't match with the password!";
                    $errCount = $errCount + 1;
               }
          }
         ...
         if (!preg_match("/^(?=.*[a-z])(?=.*[A-Z])(?!.* )(?=.*[%$#@]).+$/", $password)) {
               $passErr .= "Password must contain 8 digits with atleast a digit, a lower case and an upper case letter, atleast one of the [%$#@] and no space.";
               $password ="";
               $errCount = $errCount + 1;
              }
     ...
     if (empty($_POST["dob"])) {
      ...
      echo "<span class='error'>One or more error occurred!</span>";
           ...
           if(file_exists('data.json'))
           {
                ...
                {
                     $message = "<label class='text-success'>Your Registration is Successful!</p>";
                }
           }
           else
           {
 ...
 <html>
      ...
      <body>
           <div style="width:500px;" class="donor-info make-it-center">
                <fieldset>
